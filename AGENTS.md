...
RESEND_API_KEY="<your resend key>"   <!-- OPTIONAL for local demo — see note below -->
...
> **OTP delivery without a Resend key (for the hackathon demo):** If `RESEND_API_KEY` is empty/unset, `sendOtp()` **skips the Resend call entirely** (no noisy 401) and prints the OTP to the server console as `[CivID SSO] [DEV] OTP for <email>: <6-digit>`. This works in any non-production `NODE_ENV`. Set a valid key only if you want real email delivery.
...
| Day 5 | **GitHub Actions CI for full test suites** (`.github/workflows/tests.yml`):<br>1. **`cpgrams-backend-tests` job**: `mongo:7` service container, `npm install`, `npm run seed`, then `npm test -w apps/cpgrams-backend`. Env: `API_PUSH_KEYS=dev-push-key-12345`, `NODE_ENV=test`.<br>2. **`sso-server-tests` job**: `npm install`, `npx prisma generate` (required before `PrismaClient` import), then `npm test -w apps/sso-server`. No DB service needed — current SSO tests are self-contained. Both jobs pin Node 22 (native glob support for `node --test tests/**/*.test.js`). Triggers: push/PR to `main` + manual `workflow_dispatch`.<br>**Backend bugs found & fixed while wiring up CI (all verified by the suites):**<br>1. `grievance.js`: replaced blanket `router.use(verifyToken)` with per-route auth — the blanket middleware short-circuited ANY `/grievance/:id/<sub>` request carrying an officer JWT (no `kid`) with a 401 before documents/reminder/feedback routers could handle it.<br>2. `feedback.js` GET route: now resolves officer Bearer JWTs (`verifyOfficerToken`) and `X-Officer-Id` before falling back to citizen `verifyToken`; previously any Authorization header was forced down the citizen JWKS path → guaranteed 401 for officers.<br>3. `disclosure.js` POST /request: 201 response now includes `requestedByOfficerId` (was omitted).<br>4. `master.js`: `/master/officers` list + detail endpoints no longer leak `passwordHash` (security fix, `-passwordHash` in `.select()`). |
| Day 5 | **Fixed "View Details" link not working on citizen dashboard**: Added missing `"use client"` directive to `CaseCard.tsx`, `StatusBadge.tsx`, and `ProtectedBanner.tsx`. These shared components were imported into Client Component pages but lacked the `"use client"` directive, causing `next/link` `Link` and base-ui React primitives to not function properly in the client bundle. After adding `"use client"`, client-side navigation via "View Details" links works correctly. |
| Day 5 (Cont.) | **OTP delivery without a Resend API key (demo-friendly).** `.env` had an empty `RESEND_API_KEY`, so every OTP attempt called Resend with a placeholder key and logged a 401 (`validation_error`). Fixed `src/services/otp.js`: when no key is configured, skip the Resend call entirely, print a clear warning, and always print the OTP to the console as `[CivID SSO] [DEV] OTP for <email>: <code>` for non-production nodes. Real email still works when a valid key is set. |
| Day 5 (Cont.) | **StackOverflow-style Duplicate Detection & Issue Voting (anti-duplicate / urgency signal)**.<br>**Backend:**<br>1. `Case` model: added `votes` (int) + `voterPairwiseIds` (array) for one-vote-per-citizen tracking.<br>2. New service `src/services/duplicateDetect.js`: lightweight keyword-overlap matcher (same-category, ≥2 significant words, last-90-days, open status, never the citizen's own case). Tokenizer strips stop-words/short tokens.<br>3. `GET /grievance/suggestions?category=&q=` → similar recent open cases. Response now returns `{ suggestions[], ownDuplicate }`: `suggestions` are OTHER citizens' votable matches; `ownDuplicate` is the citizen's OWN recent matching open case, surfaced as a "You already reported this" reminder (never votable/self-votable). This lets a citizen see their own just-filed complaint while typing a repeat, alongside votable suggestions from others.<br>4. `POST /grievance/:caseId/vote` → increments `votes`, records `grievance_upvoted` audit log, returns 409 on double-vote / 400 on own case.<br>5. `officer.js` `/officer/cases` now sorts by `votes` desc (urgency-first).<br>6. `verifyToken.js`: added a **test-only bypass** (active only when `NODE_ENV=test` AND `TEST_AUTH_SECRET` set) so citizen-route API tests run without a live SSO. Never active in dev/prod.<br>7. New suite `tests/voting.test.js` (13 tests) — full backend coverage now **52 tests**.<br><br>**Frontend:**<br>8. Grievance form (`app/grievance/new/page.tsx`): debounced (600ms) live suggestions panel under the description field — fires once ~40 chars typed; shows (a) a highlighted amber **"You already reported this"** banner with a "View your case" link whenever `ownDuplicate` is returned, and (b) votable suggestions from other citizens, each showing caseId, excerpt, "N citizens already confirmed"; "This is the same — Vote" registers the vote, clears the draft, toasts, and returns to dashboard.<br>9. `CaseCard.tsx`: shows a "🔥 N confirmed" badge when `votes > 0` (surfaces urgency on citizen + officer dashboards).<br>**Voter tracking credentials:** voting on another citizen's case now mints the voter their OWN per-voter tracking password (`crypto.randomBytes(4)`, stored on new `CaseFollow` model — unique index on `(caseId, voterPairwiseId)`). The vote response returns `{ trackingCaseId, trackingPassword }`; the frontend toast shows it and auto-redirects to `/status?caseId=...&password=...`. New `GET /grievance/followed` lists all cases the citizen voted on WITH their tracking passwords (recoverable if lost). Public status routes (`POST /status/check`, `GET /status/:caseId/history`) now accept EITHER the filer's registration password OR any voter's tracking password via shared `validateAccess()` — the filer's original password is never revealed. Status existence checks relaxed to `!grievance` only (access still gated by `validateAccess`). Voting suite grew to 20 tests; full backend coverage now **58 tests**.<br>**Privacy-safe:** votes are tracked by pairwiseId only; officers never see who voted. Matching is keyword-based (no NLP/LLM, no location → no PII sent anywhere). `Case` schema got new fields (Mongo is schemaless but the model now includes them). CI workflow now sets `TEST_AUTH_SECRET`. |
