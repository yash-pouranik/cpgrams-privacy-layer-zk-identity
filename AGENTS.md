...
| Date | What changed |
|---|---|
| Day 5 (Cont.) | **Fixed "Rate Resolution" button not working (feedback status mismatch)**: The citizen "Rate Resolution" step appeared for cases closed with the **official CPGRAMS status `disposed`**, but the backend `POST /grievance/:caseId/feedback` (`src/routes/feedback.js`) only accepted the legacy alias `resolved` and returned `400 "Case must be resolved"` — so clicking Submit silently did nothing (the confirm modal just stayed open). **Fix**: acceptance condition widened to `grievance.status !== 'resolved' && grievance.status !== 'disposed'`, matching the frontend `isDisposed` guard. **Regression test added** (`tests/feedback.test.js`): a self-signed `{ sub, test:true }` citizen token POSTs a 4★ rating on a `disposed` case and expects `201`. Feedback suite 3/3 pass. This unblocks the Phase 8 loop: officer disposes → citizen rates → rating lands in the officer's public `citizenSatisfaction` on `/officers`. | Citizen rating of a disposed case now succeeds |
...
| Day 5 (Cont.) | **CivID OTP Page Redesign — Government-Grade Two-Column Verification Layout (zero functional changes)**: Rebuilt `apps/sso-server/src/views/otp.ejs` from the small centered DaisyUI card into a professional 27/73 two-column identity-verification layout. **Left panel (27%, desktop)**: full-height `#F97316` orange branding panel (subtle pixel-texture) containing the ONE CivID logo on the page (inline white interlocked-rings SVG + "CivID" wordmark), "One-Time Verification" subtitle, a subtle concentric-rings "protected identity" geometric visual, and two security assurances (identity sealed inside CivID / 5-minute single-use code). **Right workspace (73%, off-white `#FAFAF8`)**: minimal top-right utility row (Call Us 1800-11-4000, English, Accessibility), small Home → One-Time Verification breadcrumb, 3-step horizontal progress indicator (Identity Verified — green check / OTP Verification — orange current with ring / Secure Access — muted), large "Enter OTP" heading with masked-email line and preserved mock-data disclaimer, main white OTP card (border + light shadow, 12px radius, generous padding) with a prominent 16px-height letterspaced OTP input (orange focus ring, same `maxlength=6`/`pattern`/`inputmode`/`required`/`autofocus` contract), full-width orange Verify button, and Restyled "Resend OTP" as a secondary link; the existing Demo Test Code block is preserved below as a subordinate neutral `slate-50` helper card with Copy / Fill in OTP. **Functionality byte-preserved**: verify form `POST /interaction/:uid/verify` + hidden `aadhaar`, resend form `POST /interaction/:uid/login` + hidden `aadhaar`, error alert rendering, `copyOtp()`/`fillOtp()` scripts untouched, EJS locals (`uid`, `maskedEmail`, `error`, `aadhaar`, `otp`) and demo-card `if (otp)` gating unchanged; DaisyUI dependency removed from this template (plain Tailwind v4 browser CDN). Mobile: branding stacks as compact header, OTP boxes remain usable. **Verified**: EJS compile+render with error and no-OTP variants; exactly one logo; zero footer; all forms/attrs/scripts present. | UI-only redesign — backend, OTP logic, resend, and auth flows untouched |
| Phase 8 | **Officer Accountability & Public Registry (V2 Phase 8 — final phase)**:<br>1. **`apps/cpgrams-backend/src/models/OfficerMetrics.js`** [NEW]: Mongoose model — aggregated per-officer accountability metrics (total/active/resolved/overdue cases, averageResolutionDays, slaComplianceRate, citizenSatisfaction, totalFeedbackCount, appealRate, `performanceTier` enum `A+/A/B/C/NEEDS_ATTENTION`, lastUpdated). Zero citizen PII.<br>2. **`apps/cpgrams-backend/src/services/scorecard.js`** [NEW]: `computeScorecard(officerId)` aggregates live `Case` + `Feedback` data (SLA = % of resolved cases closed within 14 days; overdue = active cases older than 14 days; appeal rate over resolved cases; satisfaction from Feedback ratings), derives the tier per spec thresholds (A+ ≥95% & ≥4.5★ … NEEDS_ATTENTION <50% SLA), and upserts the `OfficerMetrics` cache best-effort. Pure `deriveTier()` export for testing.<br>3. **`officer.js`** [MODIFIED]: public `GET /officer/:officerId/scorecard` declared BEFORE the `requireOfficer` middleware — returns `{ officer, metrics }`, 404 for unknown officers, no PII.<br>4. **`master.js`** [MODIFIED]: public `GET /master/officers/public-registry` declared BEFORE `/master/officers/:officerId` (route-order fix) — returns all officers with live scorecards, filterable by `department`, sortable by `sla`/`rating`/`volume`/`resolution`; excludes `passwordHash`/`_id`/`__v`.<br>5. **`apps/frontend/components/OfficerScorecard.tsx`** [NEW]: reusable `OfficerScorecard` + `TierBadge` components — SLA progress bar (color-coded), star satisfaction display, case stat grid, tooltip tier explanations. Lucide icons only (no decorative emojis).<br>6. **`apps/frontend/app/officers/page.tsx`** [NEW]: Public Officer Performance Registry — search (name/dept/ID), department filter pills, sort selector, ranked officer cards with expandable detail (appeal rate, avg resolution, feedback count, expertise/jurisdiction badges, live-recalculation disclaimer).<br>7. **`Navbar.tsx`** [MODIFIED]: "Officer Registry" link added to desktop and mobile navigation.<br>8. **`app/page.tsx`** [MODIFIED]: "Bilateral Accountability" dark section added to landing page (id `#accountability`) — Pillar 1 Citizen Protection / Pillar 2 Officer Accountability with CTA to `/officers`.<br>**Verified**: live `200` on both endpoints (scorecard for PWD-001, registry with 10 officers, tier computed), `tsc --noEmit` exit 0, `next build` exit 0 with `/officers` prerendered, backend syntax clean. | V2 spec implementation sequence complete (Phases 1–8) |
...
| Day 5 (Cont.) | **Intelligent AI Auto-Routing Toggle & Flow**: Implemented a toggle switch (`aiAutoRoute` state) in Step 2 of `NewGrievancePage` (Next.js) allowing citizens to bypass manual department and category selections. If enabled, category is submitted as `"Unclassified (AI Triage Pending)"`. In `routes/grievance.js`, this placeholder prevents default database auto-assignment (leaving `assignedOfficerId: null`). Inside `grievanceIntelligence.worker.js`, once Agent 1 (Triage) resolves the correct department and category, they are written back to the `Case` record and local memory, allowing Agent 4 to assign the optimal recommended officer. | Complete |