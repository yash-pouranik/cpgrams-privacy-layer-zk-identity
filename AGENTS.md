...
11. **Next.js 16 (Turbopack) passes `params` to page components as a Promise** — do NOT write `export default function Pg({ params }: { params: { id: string } })` and read `params.id`. It is `undefined` at runtime (the page then fetches `/grievance/undefined`, gets a 404, and redirects). Read dynamic segments with `const { id } = useParams<{ id: string }>()` from `next/navigation`. Same convention applies to `searchParams` (use `useSearchParams()`). See Update Log "Day 4 (Cont.)".
...
| Day 4 | Grievance form (`/grievance/new`): Added "Evidence (Optional)" file picker so citizens can select an **image or PDF from their device** (JPG/PNG/GIF/WebP/PDF, up to 5MB each, multiple files) in addition to the existing external-link textarea. Files are stored as raw `File` objects with `URL.createObjectURL` previews (revoked on unmount/remove) and sent to the backend via multipart upload. |
| Day 4 (Cont.) | **Root-cause fix for "Failed to file grievance" (pre-existing, unrelated to file upload):** Every `POST /grievance` was returning `500 Internal server error` because `Case.js`'s `pre('save', function (next) { this.updatedAt = new Date(); next(); })` hook threw `TypeError: next is not a function`. Under mongoose 9.x / kareem 3.x, document `save` pre-hooks are executed via `pre.fn.apply(context, args)` and **no `next` callback is injected into the hook arguments**, so `next()` is undefined. Fixed by converting the hook to an `async function () { this.updatedAt = new Date(); }` (the version-safe Mongoose 9 idiom). Verified end-to-end with a real multipart upload returning HTTP 201. This bug blocked ALL grievance filings, not just file-attached ones. |
| Day 4 (Cont.) | Converted file evidence from base64 data-URIs to **real file storage** (multipart upload). Backend: added `multer` (`^2.2.0`), new `src/middleware/upload.js` (disk storage to `apps/cpgrams-backend/uploads/`, sanitized random filenames, image/PDF file filter, 5MB/file & 5-file limits), serves files at `/uploads` via `express.static`, and `POST /grievance` now parses `multipart/form-data` (`files` field + `urls` JSON string for external links) and stores absolute `http://localhost:5000/uploads/<file>` URLs in `evidenceUrls`. Global error handler returns 4xx for Multer/upload errors. Uploaded files are cleaned up on failure. Frontend `/grievance/new`: sends `FormData` (no manual Content-Type). No MongoDB schema change — `evidenceUrls` remains `[String]`. Also reverted the earlier `10mb` JSON body limit bump (no longer needed since files no longer travel in JSON bodies). |
| Day 4 (Cont.) | **Fixed citizen & officer "View Details" links not working on the case detail pages.** In Next.js 16 (Turbopack), `params` is passed to page components as a **Promise**; reading `params.caseId` directly yields `undefined` at runtime, so `app/case/[caseId]/page.tsx` and `app/officer/case/[caseId]/page.tsx` fetched `/grievance/undefined` → backend 404 → `router.push()` bounced the user straight back to the dashboard. Fixed both pages to read the dynamic segment via `const { caseId } = useParams<{ caseId: string }>()` (from `next/navigation`), and replaced every `params.caseId` with `caseId`. `tsc --noEmit` on `app/case/[caseId]/page.tsx` is now clean (proof `useParams` is the correct Next 16 API). |
