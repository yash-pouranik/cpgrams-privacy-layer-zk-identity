...
description: "A clean, light-mode default design inspired by Linear's typography and spacing, but inverted for a bright, accessible aesthetic. The background is pure white (#FFFFFF) with subtle gray surfaces (#F9FAFB) and borders (#E5E7EB). The signature Linear lavender-blue (#5E6AD2) is retained as the single chromatic accent. Text is crisp dark gray (#111827). Dark mode is supported as an optional toggle rather than the default."
...
Linear's inspired marketing canvas has been inverted to a bright, light-mode default — `{colors.canvas}` is #FFFFFF, pure white. On top sits a four-step surface ladder (`{colors.surface-1}` through `{colors.surface-4}`) for cards, panels, and lifted tiles, with hairline borders running from `{colors.hairline}` (#E5E7EB) up through `{colors.hairline-strong}` and `{colors.hairline-tertiary}`. Crisp dark gray text (`{colors.ink}` #111827) carries the body and headlines, while `{colors.ink-muted}` (#6B7280) supports secondary information.

The single chromatic accent remains the **Linear lavender-blue** `{colors.primary}` (#5e6ad2) — used on the brand mark, focus rings, and the primary CTA button. A lighter hover state (`{colors.primary-hover}` #828fff) and a focus-tinted variant (`{colors.primary-focus}` #5e69d1) extend the same hue. 

Display type runs a custom sans (with `SF Pro Display` fallback) at weight 500–700 with negative letter-spacing scaling from -3.0px at 80px down to 0 at body. The body family is a text cut, and a Mono is reserved for code snippets in product screenshots.

**Key Characteristics:**
- **Light-canvas marketing system** — `{colors.canvas}` (#FFFFFF) is the default page background.
- **Lavender-blue brand accent** (`{colors.primary}` #5E6AD2) — used scarcely on brand mark, focus, and the primary CTA.
- Four-step surface ladder (canvas → surface-1 → surface-2 → surface-3 → surface-4) carries hierarchy.
- Cards use `{rounded.lg}` 12px corners with 1px hairline borders — never pill, rarely 16px.
- No second chromatic color. No atmospheric gradients. No spotlight cards.
- **Dark mode** is an optional toggle, rather than the default presentation.
...
- **Surface 1** ({colors.surface-1}): One step above canvas — #F9FAFB, feature cards, pricing cards.
- **Surface 2** ({colors.surface-2}): Two steps above — #F3F4F6, hovered cards.
- **Surface 3** ({colors.surface-3}): Three steps above — #E5E7EB, tertiary backgrounds, sub-nav.
- **Surface 4** ({colors.surface-4}): Four steps above — #D1D5DB, deepest lifted surface.
- **Hairline** ({colors.hairline}): 1px borders on cards and dividers (#E5E7EB).
- **Hairline Strong** ({colors.hairline-strong}): Stronger 1px borders (#D1D5DB).
- **Hairline Tertiary** ({colors.hairline-tertiary}): Tertiary borders for nested surfaces (#9CA3AF).
- **Inverse Canvas** ({colors.inverse-canvas}): Deep dark (#010102) for dark mode or inverse elements.
- **Inverse Surface 1** ({colors.inverse-surface-1}): One step above inverse canvas (#0f1011).
- **Inverse Surface 2** ({colors.inverse-surface-2}): Two steps above inverse canvas (#141516).

### Text
- **Ink** ({colors.ink}): All headlines and emphasized body type — crisp dark gray #111827.
- **Ink Muted** ({colors.ink-muted}): Secondary type at #6B7280 — meta info, lead paragraphs.
- **Ink Subtle** ({colors.ink-subtle}): Tertiary type at #9CA3AF — deselected tabs, footer columns.
- **Ink Tertiary** ({colors.ink-tertiary}): Quaternary at #D1D5DB — disabled, footnotes.
...
The bright canvas IS the whitespace. Sections separate by lift onto surface-1 panels, not by gaps in white. Within a panel, generous `{spacing.lg}` 24px gaps between content blocks; `{spacing.section}` 96px between sections.
...
Linear's depth is carried by surface ladder + hairline borders. The brand resists drop shadows almost entirely, relying on subtle borders.

### Decorative Depth

- **Product UI screenshots** dominate as decorative depth.
- **No atmospheric gradients, no spotlight cards.**
- **Subtle edge highlight** on lifted panels.
...
- Provide a dark mode toggle as an option.

### Don't

- Don't ship a dark-mode only page.
...
- Dark mode is an optional feature for this project, implemented via Tailwind classes (e.g. `dark:bg-inverse-canvas`), though this design system documents the primary light-mode structure.
